<template>
  <div class="downloads-view">
    <el-text size="large">/files/branches/arclight</el-text>
    <el-row justify="space-around">
      <el-col v-for="(file, index) in files" :key="index" :span="6">
        <el-card>
          ...
          <template #footer>
            Last modified：{{ getLastModifiedDate(file['last-modified']) }}
          </template>
        </el-card>
      </el-col>
    </el-row>
...
async function inquireFiles() {
  ...
  files.value?.forEach(file => console.log(JSON.stringify(file)));
}
